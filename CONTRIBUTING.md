...
CRAFLET_E2E_EULA=true CRAFLET_E2E_PACKAGE="artifacts/craflet-${CRAFLET_VERSION}.tgz" pnpm test:e2e
```

In PowerShell, read the version with `$crafletVersion = node -p "require('./packages/cli/package.json').version"`, set `$env:CRAFLET_E2E_EULA = "true"` and `$env:CRAFLET_E2E_PACKAGE = "artifacts/craflet-$crafletVersion.tgz"`, then run `pnpm test:e2e`. Remove the variables from that shell afterward. Setting either variable is an explicit test setup step; the harness does not grant EULA consent.
...
`pnpm test:package` creates `artifacts/craflet-<version>.tgz`, using the version in `packages/cli/package.json`, installs it in a fresh directory, and tests both direct CLI execution and the npm-exec path. It rejects private package references, runtime npm dependencies, missing assets, and unwanted development files. The packed manifest is adjusted without removing dependencies from the development manifest.
...
npm install --global "./artifacts/craflet-${CRAFLET_VERSION}.tgz"
craflet --help
```
...
npm view craflet@0.1.0 version --registry=https://registry.npmjs.org/
```
...
gh release create v0.1.0 --repo sya-ri/craflet --verify-tag --generate-notes --title v0.1.0
```
...
```sh
gh api --method POST repos/sya-ri/craflet/rulesets --input .github/rulesets/master.json
gh api --method POST repos/sya-ri/craflet/rulesets --input .github/rulesets/release-tags.json
gh api --method PUT repos/sya-ri/craflet/environments/npm --input .github/environments/npm.json
gh api --method POST repos/sya-ri/craflet/environments/npm/deployment-branch-policies --field name='v*' --field type=tag
```
...
```sh
npm trust github craflet --file ci.yml --repo sya-ri/craflet --env npm --allow-publish --yes
npm trust list craflet
gh variable set CRAFLET_NPM_TRUSTED_PUBLISHING --repo sya-ri/craflet --body true
```
